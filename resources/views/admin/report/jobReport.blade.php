                ...
                <div class="table-responsive">
                    <table class="table mb-0" id="alldeposit">
                        ...
                        @foreach($job_reports as $job_report)
                            ...
                            <td>{{$job_report->user_id}}</td>
                            <td>{{$job_report->job_id}}</td>
                            <td>{{$job_report->report_type}}</td>
                            <td>{{$job_report->report_summery}}</td>
                            <td>{{$job_report->created_at}}</td>
                            <td>view</td>
                            <?php
                            $wordlist = \DB::table('job_reports')->where('report_type', $job_report->report_type)
                            ->get();
