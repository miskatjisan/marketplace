...
use App\Models\freelancer_report;
...
class UserJobController extends Controller
{
    ...
    {
        $all_reg = region_country::distinct()->select('region')->where('region', '!=', '')->orderBy('country_name','ASC')->get();
    ...
    public function job_report_submit(Request $request)
    {
        ...
        $new_report->buyer_id = $request->buyer_id;
        $new_report->job_id = $request->job_id;
        $new_report->report_type = $request->report_type;
    ...
    {
        $job_reports = job_report::where('buyer_id', Auth::user()->id)->get();
        $buyer_reports = apply_member_report::all()->where('applied_user_id', Auth::user()->id);
        $my_jobs = all_job::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->paginate(10);
        return view('user.job.myJobs', compact('my_jobs','job_reports','buyer_reports'));
    }
    ...
    public function my_tasks()
    {
        ...
        $job_reports = job_report::where('buyer_id', Auth::user()->id)->get();
        $buyer_reports = apply_member_report::all()->where('applied_user_id', Auth::user()->id);
        return view('user.job.myTask', compact('my_tasks','job_reports','buyer_reports'));
    }

    public function freelancer_report_submit(Request $request){
        $id = $request->id;
        $job_apply = job_apply::FindOrFail($id);
        $all_job = all_job::FindOrFail($job_apply->job_id);

        $new_report = new freelancer_report();
        $new_report->user_id = Auth::user()->id;
        $new_report->buyer_id = $all_job->user_id;
        $new_report->job_id = $all_job->job_id;
        $new_report->report_type = $request->report_type;
        $new_report->report_summery = $request->report_summery;
        $new_report->save();
        return redirect()->route('user.my.jobs')->with('success', 'Freelancer Report Submited Successfully');
    }
